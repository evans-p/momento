...
class TestCDNControllerIT extends AbstractIntegrationTest {
	...
	public void testFaultyUrl_english() throws Exception {
		mockMvc.perform(
				...
				.andExpect(content().bytes("{\"message\":\"No content found at the specified URL.\"}".getBytes()));
	}
	...
	public void testFaultyUrl_greek() throws Exception {
		mockMvc.perform(
				...
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(content().bytes("{\"message\":\"Δε βρέθηκε περιεχόμενο στο συγκεκριμένο URL.\"}".getBytes()));
	}
	...
	public void testUpload_validationErrorEnglish() throws Exception {
		mockMvc.perform(
				...
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(content().bytes("{\"messages\":{\"type.mismatch\":\"Content type and file suffix do not match.\"}}".getBytes()));
	}
	...
	public void testUpload_validationErrorGeek() throws Exception {
		mockMvc.perform(
				...
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(content().bytes("{\"messages\":{\"type.mismatch\":\"Διαφορά μεταξύ του τύπου περιεχομένου του αρχείου και της επέκτασης του.\"}}".getBytes()));
	}
